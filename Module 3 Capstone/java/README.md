...
In the database folder, you'll find the database creation scripts and a shell script called `create.sh`. Run `sh create.sh` from the database folder to create the database. The script will take care of creating a database called `tenmo`, and the psql to create and load the tables. 
...
1. All Required MVP use cases are complete and functioning. No optional use cases should be attempted until all MVP's are complete.

2. The server API follows good design principles and can be tested in Postman, and the API is integrated into the client app.

3. Each team will be given 10 minutes to demo their app to the entire class.  Each team member must take a turn presenting part of the application to the class. Assume the class is a NON-TECHNICAL product owner (meaning no technical terms). You should probably practice this in advance. After all your boss is watching and wants to make sure his is prepared.

4. Be prepared to answer questions about your code after the demo. For example, you may have to explain the annotations, or dependency injection or any other implemented feature (questions will be random. You will not need to explain 'given' code.

5. A slide should be prepared in advance of the presentation that shows your high level API design (Remember how we designed the API for the Students?) and it should be a part of your walkthrough. 

6. Follow good design and coding standards, integration tests, etc 

7. If you work on any optional use case or over the holiday weekend, your pair partner must be involved.  Do not go off and change the code without your pair partner approval. If you do work over the weekend, schedule a couple of check in times.  
